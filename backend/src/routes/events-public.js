...
const { sendEventRegistrationEmail, sendEventPendingEmail, sendInvitationConfirmedEmail, sendInvitationPendingEmail } = require('../services/email');
...
// ═══════════════════════════════════════════════════════════════════════════
// INVITATION CLAIM — Acceso público por claim_code (E1.5)
// El claim_code reemplaza al :slug — el batch resuelve event_id y user_id.
// ═══════════════════════════════════════════════════════════════════════════

const invitationClaimLimiter = rateLimit({
  windowMs: 60 * 1000, max: 10, standardHeaders: true, legacyHeaders: false,
  message: { error: 'Demasiados intentos, reintenta en un momento' },
});

// GET datos del batch + evento para renderizar el formulario
router.get('/invitation/:claim_code', invitationClaimLimiter, async (req, res) => {
  const pool = global.pool;
  try {
    const { rows } = await pool.query(
      `SELECT b.id AS batch_id, b.label, b.ticket_type, b.quota, b.claimed_count,
         ...
         JOIN events.events e ON e.id = b.event_id
        WHERE b.claim_code = $1`,
      [req.params.claim_code]
    );
    if (!rows[0]) return res.status(404).json({ error: 'Invitación no encontrada' });
    const r = rows[0];
    if (r.revoked_at) return res.status(410).json({ error: 'Esta invitación fue revocada' });
    if (r.expires_at && new Date(r.expires_at) < new Date()) {
      return res.status(410).json({ error: 'Esta invitación expiró' });
    }
    if (new Date(r.ends_at) < new Date()) {
      return res.status(410).json({ error: 'El evento ya terminó' });
    }
    if (!['published', 'live', 'draft'].includes(r.event_status)) {
    ...
    res.json({
      batch: {
        id: r.batch_id, label: r.label, ticket_type: r.ticket_type,
        quota: r.quota, claimed_count: r.claimed_count,
        slots_remaining: r.quota - r.claimed_count,
      ...
      event: {
        ...
        starts_at: r.starts_at, ends_at: r.ends_at, timezone: r.timezone,
        venue_name: r.venue_name, venue_address: r.venue_address,
        cover_image_url: r.cover_image_url, config: r.config,
        sessions,
      },
    });
  } catch (err) {
    console.error('❌ GET /invitation/:claim_code:', err);
    res.status(500).json({ error: 'Error interno' });
  }
});
...
router.post('/invitation/:claim_code/register', invitationClaimLimiter, async (req, res) => {
  ...
  try {
    const result = await withTransaction(pool, async (client) => {
      ...
      const { rows: bRows } = await client.query(
        `SELECT b.*, e.id AS ev_id, e.name AS ev_name, e.slug AS ev_slug,
                ...
                e.venue_name, e.config AS ev_config, e.status AS ev_status
           FROM events.registration_invitation_batches b
           JOIN events.events e ON e.id = b.event_id
          WHERE b.claim_code = $1
          FOR UPDATE OF b`,
      ...
      if (!bRows[0]) { const err = new Error('Invitación no encontrada'); err.httpStatus = 404; throw err; }
      const b = bRows[0];
    ...
    if (err.httpStatus) return res.status(err.httpStatus).json({ error: err.message });
    console.error('❌ POST /invitation/:claim_code/register:', err);
    res.status(500).json({ error: 'Error interno' });
  }
});
